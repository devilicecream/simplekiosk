...
function setupNavigator() {
  chrome.storage.sync.get({'position-h': 'center', 'position-v': 'bottom'}, function (result) {
    ...
    const injectedControls = '<div id="simplekiosk-navigator" class="' + classes +
    '"><button id="simplekiosk-backbutton"></button><button id="simplekiosk-homebutton"></button></div>';
...
$(document).on('click', '#simplekiosk-homebutton', event => {
	...
	chrome.storage.sync.get({'home_url': "about:newtab"}, function (result) {
	  chrome.runtime.sendMessage({goto_url: result.home_url});
	});
});
